        ...
        <div className="flex order-2 sm:order-1 items-center justify-between gap-x-10 gap-y-6 flex-wrap">
            {/* status overview items */}
            {
                items.map((item,index) => (
                    <div key={index} className="flex items-center gap-x-4 w-full sm:w-56 md:w-full lg:w-56 2xl:w-72 py-4 px-4.5 bg-white dark:bg-primary-dark rounded">
                        <svg className="size-10 sm:size-12"><use href={item.icon}></use></svg>
